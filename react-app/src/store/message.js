...
// thunk for getting all messages
export const getMessages = (conversation_id) => async(dispatch) => {
  if (conversation_id) {
    // console.log("thunk product id", conversation_id)
    const res = await fetch(`/api/conversation/${conversation_id}/messages`)
...
const messageReducer = (state = initialState, action) => {
  switch (action.type) {
    case LOAD_MESSAGES: {
      ...
      // console.log("reducer messages", action.messages)
